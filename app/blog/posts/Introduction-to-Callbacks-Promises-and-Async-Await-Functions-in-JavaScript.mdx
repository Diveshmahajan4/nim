...
summary: Understand how JavaScript handles asynchronous code using callbacks, promises, and async/await.
...
<Cover
  src="https://cdn.hashnode.com/res/hashnode/image/upload/v1685114174416/69babf6a-1893-482c-82d5-4caa634b2af4.png?w=1600&h=840&fit=crop&crop=entropy&auto=compress,format&format=webp"
  alt="Computer image"
/>
...
This causes a _pyramid of doom_ structure causing our code to grow horizontally, making it tough to manage our code.
...
![Introduction to Promises and Async and Await Promises in Javascript!✨](https://cdn.hashnode.com/res/hashnode/image/upload/v1632595509815/tx3xM2Yi9.png?w=1600&h=840&fit=crop&crop=entropy&auto=compress,format&format=webp)
...
1. We declare an async function `fetchUserData()`. Inside this function, we make an asynchronous HTTP request to the API endpoint using the `fetch()` function. The `fetch()` function returns a promise that resolves to a response object.
2. We use the `await` keyword to pause the execution and wait for the promise returned by `fetch()` to resolve. The resolved value, the response object, is stored in the `response` variable.
3. We then use another `await` keyword to pause and wait for the promise returned by the `response.json()` method, which parses the response body as JSON. The parsed JSON data is stored in the `data` variable.
4. Finally, we log the fetched user data to the console. If any error occurs during the async function execution, it will be caught in the `catch` block, allowing us to handle and log the error.
